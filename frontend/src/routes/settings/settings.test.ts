...
describe('settings page', () => {
	beforeEach(() => {
		...
		vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
			...
			if (url === '/api/settings/status') return json({ api_key_configured: true, last_successful_import: null, latest_failed_import: null, catalog_counts: { sets: 3, parts: 4, colors: 2 }, database_label: 'what2build.db', backup_schema: 'what2build.backup/v1' });
